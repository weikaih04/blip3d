...
* shape / pbr npz: ``coords`` (N, 3) uint8 -> int32, ``feats`` (N, 32) float32, normalized per channel with the
  TRELLIS.2 stage-config statistics (:func:`blip3d.models.decoders.latent_stats`, which raises when a config or key is
...
def norm_stats():
    ...
    from ..models.decoders import latent_stats
...
def load_tex(pbr_path: str, shape_path: str, shape_raw: Optional[Tuple[torch.Tensor, torch.Tensor]] = None):
    """-> {"feats": pbr latent (N, 32) pbr-normalized, "concat": the shape latent (N, 32) in the tex flow's
    shape-normalization}, both on the shape's coords. ``shape_raw``: the (coords, feats) of ``shape_path`` already read
    by the caller."""
    st = norm_stats()
    pc, pf = load_slat_raw(pbr_path)
    sc, sf = shape_raw if shape_raw is not None else load_slat_raw(shape_path)
    if not torch.equal(sc, pc):
        raise CoordsMismatch(f"shape {tuple(sc.shape)} vs pbr {tuple(pc.shape)}")
    return {"feats": normalize(pf, st.tex_mean, st.tex_std), "concat": normalize(sf, st.tex_shape_mean, st.tex_shape_std)}


def tex_placeholder(shape_item: Dict[str, torch.Tensor]):
    """Zeros on the shape's coords; ``concat`` = the shape feats (shape-normalized, as v12)."""
    feats = shape_item["feats"]
    return {"feats": torch.zeros_like(feats), "concat": feats}
